...
class AiTriggerEngine {
  ...
  private evaluateCandleClose(symbol: string, data: any) {
    if (['1h', '4h', '1d'].includes(data.interval)) {
      this.requestAnalysis(symbol, 'MEDIUM', [`${data.interval} Candle Close`]);
    }
  }

  public requestAnalysis(symbol: string, priority: AnalysisPriority, triggers: string[]) {
  ...
  private getHighestPriority(p1: AnalysisPriority, p2: AnalysisPriority): AnalysisPriority {
    const weights: Record<AnalysisPriority, number> = { CRITICAL: 4, HIGH: 3, MEDIUM: 2, LOW: 1 };
  ...
  private async processQueue() {
    ...
    try {
      ...
      AgentRunner.runAnalysis(target.symbol).catch(err => {
...
export const AiTriggerService = new AiTriggerEngine();
